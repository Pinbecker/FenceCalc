...
/** A commercial calculation always consumes one immutable design revision. */
export interface PricingDesignInput {
  id: string;
  name: string;
...
function calculateRowQuantity(
  ...
  if (row.quantityRule.kind === "CATALOG_QUANTITY") {
    return roundQuantity(quantityMap.get(row.quantityRule.quantityKey) ?? 0);
  }

  const sourceQuantity = row.quantityRule.source.kind === "MANUAL_ENTRY"
    ? manualEntryMap.get(row.code) ?? row.quantityRule.source.defaultQuantity
    : quantityMap.get(row.quantityRule.source.quantityKey) ?? 0;
  const condition = row.quantityRule.condition;
  const conditionPassed = !condition || (
    condition.operator === "GT" ? sourceQuantity > condition.value
      : condition.operator === "GTE" ? sourceQuantity >= condition.value
        : condition.operator === "LT" ? sourceQuantity < condition.value
          : condition.operator === "LTE" ? sourceQuantity <= condition.value
            : sourceQuantity === condition.value
  );
  if (!conditionPassed || sourceQuantity <= 0) {
    return 0;
  }

  const multiplied = sourceQuantity * row.quantityRule.multiplier;
  const scaled = multiplied / row.quantityRule.increment;
  const rounded = row.quantityRule.rounding === "UP" ? Math.ceil(scaled)
    : row.quantityRule.rounding === "DOWN" ? Math.floor(scaled)
      : row.quantityRule.rounding === "NEAREST" ? Math.round(scaled)
        : scaled;
  return roundQuantity(Math.max(row.quantityRule.minimum, rounded * row.quantityRule.increment));
}
...
function buildCommercialRows(workbook: EstimateWorkbook): EstimateRow[] {
  ...
  maybePush(
    "commercial:material-markup",
    "Material markup",
    workbook.totals.materialMarkupPercent ?? 0,
    "%",
    workbook.totals.materialMarkupPercent
      ? (workbook.totals.materialMarkupAmount ?? 0) / workbook.totals.materialMarkupPercent
      : 0,
    workbook.totals.materialMarkupAmount ?? 0,
  );
  maybePush(
    "commercial:labour-overhead",
    "Labour overhead",
    workbook.totals.labourOverheadPercent ?? 0,
    "%",
    workbook.totals.labourOverheadPercent
      ? (workbook.totals.labourOverheadAmount ?? 0) / workbook.totals.labourOverheadPercent
      : 0,
    workbook.totals.labourOverheadAmount ?? 0,
  );
  maybePush(
    "commercial:labour-markup",
    "Labour markup",
    workbook.totals.labourMarkupPercent ?? 0,
    "%",
    workbook.totals.labourMarkupPercent
      ? (workbook.totals.labourMarkupAmount ?? 0) / workbook.totals.labourMarkupPercent
      : 0,
    workbook.totals.labourMarkupAmount ?? 0,
  );
...
function buildWarnings(drawing: PricingDesignInput): EstimateWarning[] {
...
export function buildCommercialEstimateCalculation(
  drawings: PricingDesignInput[],
  ...
  if (drawings.length === 0) {
    throw new Error("At least one design revision is required for calculation");
  }
  ...
  for (const drawing of drawings) {
    for (const [key, quantity] of buildCatalogQuantityMap(drawing, options)) {
      incrementQuantity(quantityMap, key, quantity);
    }
  }
  ...
  const labourOverheadAmount = roundMoney(
    labourSubtotal * ((settings.labourOverheadPercent ?? 0) / 100),
  );
  const materialMarkupAmount = roundMoney(
    materialsSubtotal * ((settings.materialMarkupPercent ?? 0) / 100),
  );
  const labourMarkupAmount = roundMoney(
    (labourSubtotal + labourOverheadAmount) * ((settings.labourMarkupPercent ?? 0) / 100),
  );
  const labourBaseForDays = roundMoney(
    labourSubtotal + labourOverheadAmount + distributionCharge + hardDigTotal + clearSpoilsTotal,
  );
  const labourDays =
  ...
  const workbook: EstimateWorkbook = {
    ...
    totals: {
      ...
      labourMarkupPercent: settings.labourMarkupPercent,
      labourMarkupAmount,
      distributionCharge,
      travelDays: labourDays,
      ...
      grandTotal: roundMoney(
        materialsSubtotal + materialMarkupAmount + labourBaseForDays + labourMarkupAmount + travelTotal + markupTotal,
      ),
    },
  };
  ...
  return {
    engineVersion: "2026-08-commercial-v1",
    designs: drawings.map((drawing) => ({
      drawingId: drawing.id,
      drawingName: drawing.name,
      drawingRevisionId: drawing.revisionId ?? drawing.id,
    ...
    totals: {
      ...
      labourCost: roundMoney(
        labourBaseForDays + materialMarkupAmount + labourMarkupAmount + travelTotal + markupTotal + ancillaryLabourTotal,
      ),
      totalCost: roundMoney(
        workbook.totals.grandTotal + ancillaryMaterialTotal + ancillaryLabourTotal,
      ),
    },
    warnings: drawings.flatMap((drawing) =>
      buildWarnings(drawing).map((warning) => ({
        ...warning,
        message: drawings.length > 1 ? `${drawing.name}: ${warning.message}` : warning.message,
      })),
    ),
    pricingSnapshot:
...
export function buildWorkbookPricedEstimate(
  ...
  const calculation = buildCommercialEstimateCalculation(
    [drawing],
    pricingConfig,
    ancillaryItems,
    manualEntries,
  ...
  return {
    drawing: {
      drawingId: drawing.id,
      drawingName: drawing.name,
      customerId: drawing.customerId ?? null,
      customerName: drawing.customerName ?? "",
    },
    groups: calculation.groups,
    ancillaryItems: calculation.ancillaryItems,
    manualEntries: calculation.manualEntries,
    workbook: calculation.workbook,
