...
export function buildPricedEstimate(
  ...
  const hasUnsupportedFenceSystem = drawing.layout.segments.some((segment) => segment.spec.system !== "TWIN_BAR");

  if (hasUnsupportedFenceSystem) {
    warnings.push({
      code: "UNSUPPORTED_FENCE_SYSTEM",
      message: "Automatic pricing is only enabled for Twin Bar drawings in this deployment. Quote non-Twin Bar layouts manually."
    });
  }
  ...
  if (junctionAndInlineJoinCount > 0) {
    warnings.push({
      ...
      message: `${junctionAndInlineJoinCount} inline join / junction posts need manual review because automated pricing is not enabled for them yet.`
    });
  }
  ...
  if (drawing.estimate.corners.unclassified > 0) {
    warnings.push({
      ...
      message: `${drawing.estimate.corners.unclassified} corner posts remain unclassified for this layout and must be reviewed manually before quoting.`
    });
  }
  ...
  if (gateCounts.customGates > 0) {
    warnings.push({
      code: "CUSTOM_GATES",
      message: `${gateCounts.customGates} custom gates are excluded from automatic pricing and need a manual quote.`
    });
  }
  ...
  if (drawing.layout.segments.length > 0) {
    warnings.push({
      code: "FIXINGS_EXCLUDED",
      message: "Fixings are excluded from automatic pricing in this deployment and should be reviewed manually."
    });
  }
  ...
  const pricingSnapshot =
    pricingConfig.updatedByUserId === null
      ...
      : {
          ...
          source: "COMPANY_CONFIG" as const
        };
