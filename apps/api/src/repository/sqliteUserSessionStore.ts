...
import type { CompanyRecord, CompanyUserRecord, UserRole } from "@fence-estimator/contracts";
...
export class SqliteUserSessionStore {
  ...
  public bootstrapOwnerAccount(
    input: BootstrapOwnerAccountInput,
  ): { company: CompanyRecord; user: CompanyUserRecord } | null {
  ...
  public updateUserPassword(
    ...
    this.database
      .prepare(
        "UPDATE users SET password_hash = ?, password_salt = ? WHERE id = ? AND company_id = ?",
      )
  ...
  public revokeSession(tokenHash: string, revokedAtIso: string): void {
    this.database
      .prepare("UPDATE sessions SET revoked_at_iso = ? WHERE token_hash = ?")
      .run(revokedAtIso, tokenHash);
  }
