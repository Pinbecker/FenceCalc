...
export async function createQuoteForCompany(
  ...
  if (!approved) {
    return { ok: false, status: 409, message: "Create a quote from the current approved estimate version" };
  }
...
export async function updateQuoteVersionForCompany(
  ...
  if (patch.estimateVersionId) {
    ...
    if (!approved || approved.estimate.id !== quote.estimateId) {
      return { ok: false, status: 409, message: "Quote versions must use an approved version of the same estimate" };
    }
  }
  const now = new Date().toISOString();
  ...
  const updated = await repository.updateQuoteVersion({
    ...
    companyId: context.company.id,
    ...(patch.estimateVersionId !== undefined ? { estimateVersionId: patch.estimateVersionId } : {}),
    ...
    ...((patch.displayMode !== undefined || patch.vatRate !== undefined || patch.estimateVersionId !== undefined)
      ? {
          presentation: buildQuotePresentation(
...
export async function setQuoteVersionStatusForCompany(
  ...
  if (!QUOTE_TRANSITIONS[version.status].includes(status)) {
    return { ok: false, status: 409, message: `Cannot move a quote from ${version.status} to ${status}` };
  }
  if (status === "ISSUED") {
    const approved = await getApprovedEstimateVersion(
      repository,
    ...
    if (!approved || approved.estimate.id !== quote.estimateId) {
      return { ok: false, status: 409, message: "The linked estimate must still be approved before issue" };
    }
...
export async function createQuoteVersionForCompany(
  ...
  if (!parent || parent.status === "DRAFT" || parent.status === "ACCEPTED") {
    return {
      ok: false,
      status: 409,
      message: parent?.status === "ACCEPTED"
  ...
  if (!approved || approved.estimate.id !== quote.estimateId) {
    return { ok: false, status: 409, message: "Select an approved version of the quote's estimate" };
  }
  const now = new Date().toISOString();
