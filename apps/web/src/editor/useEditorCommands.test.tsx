...
function createCommandHarness(overrides: Partial<CommandHarnessState> = {}) {
  ...
  const resolveFloodlightColumnPreview = vi.fn((point: PointMm) => ({
    segment: state.layout.segments[0]!,
    segmentLengthMm: distanceMm(state.layout.segments[0]!.start, state.layout.segments[0]!.end),
    offsetMm: point.x,
    point: { x: point.x, y: state.layout.segments[0]!.start.y },
  ...
  const applyFloodlightColumnPlacements = (
    updater: (previous: NonNullable<LayoutModel["floodlightColumns"]>) => NonNullable<LayoutModel["floodlightColumns"]>
  ) => {
    state.layout = {
      ...state.layout,
      floodlightColumns: updater(state.layout.floodlightColumns ?? [])
    };
  };
  ...
  function rerender() {
    ...
    const resolvedFloodlightColumnPlacements = resolveFloodlightColumnPlacements(
      segmentsById,
      state.layout.floodlightColumns ?? []
    );
    ...
    const resolvedFloodlightColumnById = new Map(
      resolvedFloodlightColumnPlacements.map((floodlightColumn) => [floodlightColumn.id, floodlightColumn] as const)
    );
    ...
    commands = renderHookServer(() =>
      useEditorCommands({
        ...
        floodlightColumnPreview: state.floodlightColumnPreview,
...
describe("useEditorCommands", () => {
  ...
  it("handles draw, rectangle, gate, recess, selection, and pan mouse down flows", () => {
    ...
    harness.state.layout.floodlightColumns = [{ id: "fc-existing", segmentId: "s1", offsetMm: 2100, facing: "LEFT" }];
    ...
    expect(harness.state.layout.floodlightColumns).toHaveLength(1);
    ...
    expect(currentSegmentIds.has(harness.state.layout.floodlightColumns?.[0]?.segmentId ?? "")).toBe(true);
  ...
  it("handles dragging, pointer updates, wheel, deletion, clearing, and reset flows", () => {
    ...
    harness.state.layout.basketballPosts = [
      { id: "bp-1", segmentId: "s1", offsetMm: 2525, facing: "LEFT" }
    ];
    ...
    expect(harness.state.layout.basketballPosts?.find((placement) => placement.id === "bp-1")?.offsetMm).toBe(2550);
    ...
    harness.state.layout.floodlightColumns = [
      { id: "fc-1", segmentId: "s1", offsetMm: 1800, facing: "LEFT" },
      { id: "fc-2", segmentId: "s1", offsetMm: 3200, facing: "RIGHT" }
    ];
    harness.state.selectedFloodlightColumnId = "fc-1";
    harness.rerender();
    harness.commands.startSelectedFloodlightColumnDrag("fc-1");
    expect(harness.state.activeFloodlightColumnDrag?.floodlightColumnId).toBe("fc-1");
    harness.state.floodlightColumnPreview = {
      segment: harness.state.layout.segments[0]!,
      segmentLengthMm: 5000,
      offsetMm: 3300,
      point: { x: 3300, y: 0 },
      ...
      normal: { x: 0, y: -1 },
      facing: "RIGHT",
      targetPoint: { x: 3300, y: 0 },
      snapMeta: { kind: "ALIGNMENT", label: "Aligned column" },
      alignmentGuide: {
        anchorPoint: { x: 3300, y: 0 },
        targetPoint: { x: 3300, y: 0 }
      }
    };
    harness.stage.pointer = { x: 2050, y: 0 };
    harness.rerender();
    harness.commands.onStageMouseMove();
    expect(harness.state.layout.floodlightColumns?.find((placement) => placement.id === "fc-1")?.offsetMm).toBe(3300);
    expect(harness.state.layout.floodlightColumns?.find((placement) => placement.id === "fc-1")?.facing).toBe("RIGHT");

    harness.state.activeFloodlightColumnDrag = null;
    harness.state.selectedFloodlightColumnId = null;
    harness.state.floodlightColumnPreview = null;
    ...
    harness.state.layout = {
      segments: buildBaseSegments(),
      gates: [],
      basketballPosts: [{ id: "bp-1", segmentId: "s1", offsetMm: 2525, facing: "LEFT" }],
      floodlightColumns: []
    };
    ...
    harness.state.layout = {
      ...
      gates: [],
      basketballPosts: [],
      floodlightColumns: [{ id: "fc-1", segmentId: "s1", offsetMm: 1500, facing: "LEFT" }]
    };
    harness.state.selectedBasketballPostId = null;
    harness.state.selectedFloodlightColumnId = "fc-1";
    harness.rerender();
    harness.commands.handleDeleteSelection();
    expect(harness.state.layout.floodlightColumns).toHaveLength(0);
  ...
  it("inserts floodlight columns in floodlight mode", () => {
    const uuidSpy = vi
      .spyOn(globalThis.crypto, "randomUUID")
      .mockReturnValueOnce("88888888-8888-4888-8888-888888888888");
    const baseSegment = buildBaseSegments()[0]!;
    const harness = createCommandHarness({
      ...
      floodlightColumnPreview: {
        ...
        normal: { x: 0, y: -1 },
        facing: "RIGHT",
        targetPoint: { x: 2500, y: 0 },
        snapMeta: { kind: "CENTERED", label: "Centered" }
      }
    });

    harness.commands.onStageMouseDown(createMouseEvent(0, harness.stage));

    expect(harness.state.layout.floodlightColumns).toEqual([
      {
